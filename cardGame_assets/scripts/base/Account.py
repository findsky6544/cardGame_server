...
import datetime
from BAN_INFOS import BanInfos
from BAN_INFOS import BanInfosList
...
class Account(KBEngine.Proxy):
	def __init__(self):
		...
		self.cellData["winCountC"] = self.winCount
		self.cellData["loseCountC"] = self.loseCount
	...
	def reqSetName(self, name):
		...
		if(self.name != ''):
			DEBUG_MSG("Account base::name is exist,name:%s" % self.name)
			retcode = 25
			self.client.onSetNameFailed(retcode, name)
			return
		if(name == ''):
			DEBUG_MSG("Account base::name is null")
			retcode = 26
			self.client.onSetNameFailed(retcode, name)
			return
		...
		self.name = name
		self.cellData["nameC"] = self.name
		DEBUG_MSG("Account base::set name: %s." % name)
	...
	def reqCreateRoom(self, name, intro, password):
		...
		createRoomResult = KBEngine.globalData["Hall"].reqCreateRoom(self,name,intro,password)
		self.client.onCreateRoomFailed(createRoomResult)
	...
	def onClientEnabled(self):
		...
		cell部分。
		"""
		INFO_MSG("Account base::onClientEnabled:id:%s,name:%s,winCount:%s,loseCount:%s" % (self.id, self.name,self.winCount,self.loseCount))
		...
		if len(self.bansList)>0:
			...
			if ban["state"] == 1:
				...
				if nowDateTime > ban["startTime"] and nowDateTime <ban["finishTime"]:
