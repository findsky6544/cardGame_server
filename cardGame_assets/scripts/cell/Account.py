...
class Account(KBEngine.Entity, EntityCommon):
	...
	def reqEditRoomInfo(self,callerEntityId,roomKey,name,intro):
		...
		editRoomInfoResult = 0
		editRoomInfoResult = self.getCurrRoom().reqEditRoomInfo(roomKey,name,intro)
		if editRoomInfoResult != 0:
			self.client.onEditRoomInfoFailed(editRoomInfoResult)
	...
	def sendChatMessage(self,callerEntityId,message):
		DEBUG_MSG("account cell:chat:%s:%s" % (self.nameC,message))
		self.getCurrRoom().sendChatMessage(self.nameC,message)

	def sendVoice(self,callerEntityId,voice):
		DEBUG_MSG("account cell:sendVoice:%s,voice:%s" % (self.nameC,voice))
		self.allClients.playVoice(voice)
