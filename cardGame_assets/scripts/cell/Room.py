...
class Room(KBEngine.Entity):
	...
	def leaveRoom(self, entityCall):
		...
		self.message += entityCall.nameC+"退出了房间。\n"
		self.setNewSeatsData()
	...
	def reqGameInit(self):
		...
		if(self.hasInit < 2):
			...
			#确定起始叫牌位
			self.raiseIndex = self.openCardIndex // 17
			#确定明牌与底牌
	...
	def gameOver(self,callerEntityId):
		...
		for playerId in self.players:
			baseEntityCall = self.players[playerId].base
			#地主赢了
			if self.players[playerId].seatIndex == self.landlordIndex:
				if self.players[callerEntityId].seatIndex == self.landlordIndex:
					#地主胜场+1
					baseEntityCall.addWinCount()
					self.players[playerId].winCountC += 1
					self.players[playerId].gameResult = 0
				else:
					#农民负场+1
					baseEntityCall.addLoseCount()
					self.players[playerId].loseCountC += 1
	...
	def sendChatMessage(self,accountName,message):
		DEBUG_MSG("room chat:%s:%s" % (accountName,message))
		self.message += accountName+":"+message+"\n"
	...
	def onEnter(self, entityCall):
		...
		for index in range(3):
			if not self.playersData.__contains__(index) or self.playersData[index]["id"] == 0:
				...
				self.playersData[index] = {"id":entityCall.id,"name":entityCall.nameC,"winCount":entityCall.winCountC,"loseCount":entityCall.loseCountC,"isReady":0,"seatIndex":index,"isRoomMamster":0}
				if len(self.playersData) == 1:
					self.playersData[index]["isRoomMamster"] = 1
				break
		self.message += entityCall.nameC+"进入了房间。\n"
		self.setNewSeatsData()
